...
def runSubQueue(threadName, myQueue, stopEvent, mc, pending):
	...
	while (not stopEvent.is_set()):
		...
		if line['action']=='newTop':
			...
			while True:
				try:
					...
					if newline['action']=='update' and line['type']=='mnode':
		...
		if counter/100>checkpoint:
			...
			print currentTime,threadName,counter,"\tinQ:%d\tq/sec:%.3f\ttopFail:%d\tbranchFail:%d\tupdateFail:%d"%(myQueue.qsize(),qps,topFail,branchFail,updateFail)
