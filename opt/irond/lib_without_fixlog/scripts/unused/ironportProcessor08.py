...
def runSubQueue(threadName, myQueue, stopEvent, mc, pending):
	...
	while (not stopEvent.is_set()):
		...
		if counter/1000>checkpoint:
			...
			print currentTime,threadName,'{0: >9}'.format(counter),"inQ:%s q/s:%.0f\t\ttopFail:%d\tbranchFail:%d\tupdateFail:%d\tdocmiss:%d\tunknown:%d"%('{0: >7}'.format(myQueue.qsize()),qps,topFail,branchFail,updateFail,docmiss,unknownError)
...
def runICIDQueue(threadName, myQueue, stopEvent, mc, icPending):
	...
	while (not stopEvent.is_set()):
		...
		if counter/1000>checkpoint:
			...
			currentTime=str(datetime.datetime.now()).rsplit(":",1)[0]
			print currentTime,threadName,'{0: >9}'.format(counter),"inQ:%s q/s:%.0f\t\tfailCount:%d"%('{0: >7}'.format(myQueue.qsize()),qps,failCount)
...
def runMyQueue(threadName, myQueue, stopEvent):
	...
	while (not stopEvent.is_set()):
		...
		if counter%4000==0 or (counter<1000 and counter%100==0): # or (counter<10 and counter%2==0):
			...
			currentTime=str(datetime.datetime.now()).rsplit(":",1)[0]
			print currentTime,threadName+".M",'{0: >9}'.format(counter),"inQ:%s q/s:%.0f\tcps:%+.1f\tnotFound:%d,%d\tmiss:%d(%.2f)\tpending:%d\ticp:%d"%('{0: >7}'.format(inQueueNow),qps,qChangeRate,notFoundCount,watch,missCount,float(missCount*100)/counter,len(pending),len(icPending))
			...
			#check all my thread
			dead=False
			for aThread in subThread:
				if not aThread.isAlive():
					print "A SUB THREAD ARE DEAD"
					dead=True
					break
			if not icThread.isAlive():
				print "A IC THREAD ARE DEAD"
				dead=True
			if not dcThread.isAlive():
				print "A DC THREAD ARE DEAD"
				dead=True
...
if __name__ == "__main__":
	...
	sys.stderr.flush()
	se = file('/var/log/ironportProcessor.err', 'a+', 0)
