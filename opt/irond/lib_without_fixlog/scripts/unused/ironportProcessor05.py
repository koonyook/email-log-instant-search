...
def runSubQueue(threadName, myQueue, stopEvent, mc, pending):
	...
	while (not stopEvent.is_set()):
		...
		if counter/1000>checkpoint:
			...
			print currentTime,threadName,counter,"\tinQ:%d\tq/sec:%.3f\ttopFail:%d\tbranchFail:%d\tupdateFail:%d\tdocmiss:%d\tunknown:%d"%(myQueue.qsize(),qps,topFail,branchFail,updateFail,docmiss,unknownError)
	...
	while (not stopEvent.is_set()):
		...
		if line['action']=='newICID':
			...
			if result['ok']:
				...
				icPending.pop(line['indexName']+'/'+line['value']['ICID'])
			else:
				icPending.pop(line['indexName']+'/'+line['value']['ICID'])
		...
		if counter/200>checkpoint:
			...
			currentTime=str(datetime.datetime.now()).rsplit(":",1)[0]
			print currentTime,threadName,counter,"\tinQ:%d\tq/sec:%.3f\tupdateFail:%d"%(myQueue.qsize(),qps,updateFail)
...
def runMyQueue(threadName, myQueue, stopEvent):
	...
	icPending={}
	icQueue=Queue.Queue()
	icThread=threading.Thread(target=runICIDQueue, args=(threadName+'.IC',icQueue,stopEvent,mc,icPending))
	icThread.start()
	...
	while (not stopEvent.is_set()):
		...
		if counter%1000==0 or (counter<1000 and counter%100==0): # or (counter<10 and counter%2==0):
			...
			currentTime=str(datetime.datetime.now()).rsplit(":",1)[0]
			print currentTime,threadName,counter,"\tinQ:%d\tq/sec:%.3f\tnotFound:%d\tmiss:%d(%.2f)\tpending:%d\ticp:%d"%(myQueue.qsize(),qps,notFoundCount,missCount,float(missCount*100)/counter,len(pending),len(icPending))
